...
def heart_tracks
  ...
  -- where tracks.song LIKE '%Hearts%'
  -- group by albums.asin
  -- order by count(tracks.*) desc, albums.title asc
  SELECT
        albums.title,
        COUNT(tracks.*)
      FROM
...
def song_title_counts
  ...
  execute(<<-SQL)
  select distinct tracks.song, count(distinct albums.*)
  from tracks
  join albums on albums.asin = tracks.album
...
def best_value
  ...
  # of tracks.
  execute(<<-SQL)
  select distinct albums.title, albums.price, count(tracks.*)
  from albums
  join tracks on albums.asin = tracks.album
  group by albums.asin
...
def expensive_tastes
  ...
  from styles
  join (select albums.*, count(tracks.*) as count
    from tracks
    join albums on albums.asin = tracks.album
